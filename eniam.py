...
def main():
    if len(sys.argv)>1 and sys.argv[1] == "sub":
        subprocess.call(["docker", "run", "-it", "eniam", "/root/subsyntax"]+sys.argv[2:])
    elif len(sys.argv)>1 and sys.argv[1] == "lex":
        subprocess.call(["docker", "run", "-it", "eniam", "/root/print_lexicon"]+sys.argv[2:])
    elif len(sys.argv)>1 and sys.argv[1] == "dom":
        subprocess.call(["docker", "run", "-it", "eniam", "/root/domparser"]+sys.argv[2:])
    else:
