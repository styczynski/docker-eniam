...
let string_of_token_env_conll n t =
  Printf.sprintf "%d\t%s\t%s\t%s\t%s" n t.ENIAMtokenizerTypes.orth
    (ENIAMtokens.get_lemma t.ENIAMtokenizerTypes.token)
    (ENIAMtokens.get_pos t.ENIAMtokenizerTypes.token)
    (ENIAMtokens.get_interp t.ENIAMtokenizerTypes.token)

let token_list_conll paths (*last*) =
  let l,_,_ = Xlist.fold paths ([],0,0) (fun (paths,beg,n) t ->
    if t.ENIAMtokenizerTypes.beg = beg then (string_of_token_env_conll n t) :: paths, t.ENIAMtokenizerTypes.beg, n
    else (string_of_token_env_conll (n+1) t) :: paths, t.ENIAMtokenizerTypes.beg, n+1) in
  String.concat "\n" (List.rev l)
  (* ^ (if last < 0 then "" else Printf.sprintf "\nlast=%d" last) *)
...
let struct_sentence spaces t paths last =
  ...
  String.concat "\n" (Xlist.map (List.sort compare paths) (fun (id,lnode,rnode) ->
    ...
    sprintf "%s%3d %5d %5d %s %s %s" spaces
      id lnode rnode t2.ENIAMtokenizerTypes.cat t2.ENIAMtokenizerTypes.orth (ENIAMtokens.string_of_token t2.ENIAMtokenizerTypes.token))) ^
...
let dep_sentence spaces t paths =
  ...
  String.concat "\n" (List.rev (Int.fold 0 (Array.length paths - 1) [] (fun l conll_id ->
    ...
    (sprintf "%s%3d %8d %s %s %s %s" spaces
      id conll_id sl sem t2.ENIAMtokenizerTypes.orth (ENIAMtokens.string_of_token t2.ENIAMtokenizerTypes.token)) :: l)))
