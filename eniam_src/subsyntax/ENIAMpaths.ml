...
(* let to_string_indexed (paths,last) =
  String.concat "\n" (Xlist.map paths (fun (i,j,t) ->
    Printf.sprintf "%2d %2d %s" i j (ENIAMtokens.string_of_tokens 0 (Token t))))
  ^ Printf.sprintf "\nlast=%d" last *)
...
let rec uniq_rec rev = function
  ...
  | [p] -> List.rev (p :: rev)
  | p :: r :: l -> 
(*      Printf.printf "uniq_rec 1: %s\n" (ENIAMtokens.string_of_token_env p);
      Printf.printf "uniq_rec 2: %s\n" (ENIAMtokens.string_of_token_env r);
...
let remove_inaccessible_tokens paths beg last =
  ...
  if not (IntSet.mem set last) then raise ENIAMsubsyntaxTypes.BrokenPaths else
...
let remove_category cat paths =
  List.rev (Xlist.fold paths [] (fun paths t ->
    if t.cat = cat then paths else t :: paths))
...
let process_interpunction_token beg next t = 
  if t.beg = beg then 
    ...
    if t.next = next then match t.token with
      ...
      | Lemma(",","conj",[[]]) -> t :: (create_clause_end_beg t.beg t.len t.next t.orth)
...
let merge_lemmata l =
  ...
  let map = StringMap.map map (fun (lemma,quantity,interps,status) ->
    ...
    Xlist.fold interps StringMap.empty (fun map interp ->
      Xlist.fold (ENIAMtagset.parse interp) map (fun map (pos,tags) ->
...
let merge_lemmata l =
  ...
  if !ENIAMsubsyntaxTypes.merge_lemmata then
    ...
    let map = StringMap.map map (fun (lemma,quantity,interps(*,status*)) ->
      lemma,
      Xlist.fold interps StringMap.empty (fun map interp ->
        Xlist.fold (ENIAMtagset.parse (map_depr_interp interp)) map (fun map (pos,tags) ->
          StringMap.add_inc map pos [tags] (fun l -> tags :: l))),
      max 1 (quantity / Xlist.size interps)(*,
      status*)) in
    ...
    let l = Xlist.rev_map l (fun t ->
      ...
      Xlist.fold (ENIAMtagset.parse t.ENIAMinflexion.interp) StringMap.empty (fun map (pos,tags) ->
        StringMap.add_inc map pos [tags] (fun l -> tags :: l)),
  ...
  List.rev (Xlist.fold (lemmatize_rec [] paths) [] (fun paths t ->
    match t.token with
      Lemma(lemma,cat,_) -> if (Xlist.mem t.attrs LemmNotVal || Xlist.mem t.attrs TokNotFound) &&
        (Xlist.mem t.attrs HasAglSuffix) && not !ENIAMsubsyntaxTypes.merge_lemmata then (
...
(*let rec lemmatize_tokens paths next_id = function
    Token({token=Dig(v,cat)} as t)-> PrePaths.add_edge paths t.beg next_id t.orth v (ENIAMtagset.parse cat) t.beg t.len
  | Token({token=Lemma(lemma,interp)} as t) -> PrePaths.add_edge paths t.beg next_id t.orth lemma (ENIAMtagset.parse interp) t.beg t.len
  | Token({token=Interp lemma} as t) -> PrePaths.add_edge paths t.beg next_id t.orth lemma (ENIAMtagset.parse "interp") t.beg t.len
  | Token({token=AllSmall s} as t) ->
       Xlist.fold (ENIAMinflexion.get_interpretations s) paths (fun paths (lemma,postags) ->
         PrePaths.add_edge paths t.beg next_id t.orth lemma (ENIAMtagset.parse postags) t.beg t.len)
...
(*let rec lemmatize_paths_tokens paths = function (* FIXME: uzgodnić postać lematów *)
    Token({token=Dig(v,cat)} as t)-> PrePaths.add_edge paths t.beg t.next t.orth v (ENIAMtagset.parse cat) t.attrs t.beg t.len
  | Token({token=Lemma(lemma,interp)} as t) ->
      ...
      PrePaths.add_edge paths t.beg t.next t.orth lemma (ENIAMtagset.parse interp) t.attrs t.beg t.len
  | Token({token=Interp lemma} as t) -> PrePaths.add_edge paths t.beg t.next t.orth lemma (ENIAMtagset.parse "interp") t.attrs t.beg t.len
...
(*  | Token({token=AllSmall s} as t) ->
       Xlist.fold (ENIAMinflexion.get_interpretations s) paths (fun paths (lemma,postags) ->
         PrePaths.add_edge paths t.beg t.next t.orth lemma (ENIAMtagset.parse postags) t.beg t.len)
