...
let string_of_bipaths bipaths =
  let bipaths = Xlist.map bipaths (fun paths ->
    let paths = Xlist.map paths (fun t ->
      Printf.sprintf "%s-%d-%d" (if t.orth = "" then ENIAMtokens.get_lemma t.token else t.orth) t.beg t.next) in
...
let process_paths tokens beg next paths last = (* FIXME: trzeba osobno obsługiwać przypadek z <query> i </query> bo teraz trawia wewnątrz <sentence> *)
  let paths = ENIAMpaths.process_interpunction beg next paths in
  let paths = if !default_category_flag then paths else ENIAMpaths.remove_category "X" paths in
  let paths = if !default_category_flag then paths else ENIAMpaths.remove_category "MWEcomponent" paths in
  let paths = Xlist.sort paths ENIAMpaths.compare_token_record in
  let paths,f = 
    try ENIAMpaths.remove_inaccessible_tokens paths beg last,true 
    with BrokenPaths -> [],false in
  if f then 
    let paths = Xlist.sort paths ENIAMpaths.compare_token_record in
    let paths,_ = ENIAMpaths.uniq (Xlist.sort paths ENIAMpaths.compare_token_record,0) in
    let paths = make_ids tokens paths in
    let paths,last = prepare_indexes paths in
    ENIAM,StructSentence(paths,last) 
...
let split_into_sentences pid paragraph tokens paths =
  ...
  let bipaths = ENIAMpaths.biconnected_compontents paths in
