...
let rec remove_symbols i text =
  if String.length text > 0 && List.mem (String.get text 0) symbols
  then remove_symbols (i+100) (String.sub text 1 (String.length text - 1))
  else i, text

let rec establish_for_token i text tokens = function
    (id,_,_) :: t as l->
      let h = ExtArray.get tokens id in
      let i, text = remove_symbols i text in
