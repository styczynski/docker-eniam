...
type ntoken = {nid: int; north: string; nlemma: string; ncat: string; ninterp: string list}

let get_interp = function
    Interp _ -> []
  | Lemma(_,_,[interp]) ->
     Xlist.map interp (function [x] -> x | [x;y] -> x ^ "." ^ y | _ -> failwith "get_interp")
  | _ -> failwith "get_interp"

let simple_allign_conll tokens =
  let tokens = List.tl (Array.to_list (Array.mapi (fun i t -> i,t) (ExtArray.to_array tokens))) in
  List.rev (Xlist.rev_map tokens (fun (id,t) ->
      {nid=id;north=t.orth;
       nlemma=ENIAMtokens.get_lemma t.token;
       ncat=ENIAMtokens.get_pos t.token;
       ninterp=get_interp t.token}))
...
let classify_variants l =
  ...
  | [[a];[b;"eś"]] -> if String.sub a 0 (String.length a - 3) = b && String.sub a (String.length a - 3) 3 = "eś" then Disamb else Unknown
  | [[a];[b;"śmy"]] -> if String.sub a 0 (String.length a - 4) = b && String.sub a (String.length a - 4) 4 = "śmy" then Disamb else Unknown
  | [[a];[b;"eśmy"]] -> if String.sub a 0 (String.length a - 5) = b && String.sub a (String.length a - 5) 5 = "eśmy" then Disamb else Unknown
  | [[a];[b;"ście"]] -> if String.sub a 0 (String.length a - 5) = b && String.sub a (String.length a - 5) 5 = "ście" then Disamb else Unknown
  | [[a];[b;"eście"]] -> if String.sub a 0 (String.length a - 6) = b && String.sub a (String.length a - 6) 6 = "eście" then Disamb else Unknown
  ...
  | [[b;"m"];[c;"em"]] -> if String.sub b 0 (String.length b - 1) = c && String.sub b (String.length b - 1) 1 = "e" then Disamb else Unknown
  | [[b;"ś"];[c;"eś"]] -> if String.sub b 0 (String.length b - 1) = c && String.sub b (String.length b - 1) 1 = "e" then Disamb else Unknown
  | [[b;"śmy"];[c;"eśmy"]] -> if String.sub b 0 (String.length b - 1) = c && String.sub b (String.length b - 1) 1 = "e" then Disamb else Unknown
  | [[b;"ście"];[c;"eście"]] -> if String.sub b 0 (String.length b - 1) = c && String.sub b (String.length b - 1) 1 = "e" then Disamb else Unknown
  | [[b;"m"];[c;"by";"m"]] -> if String.sub b 0 (String.length b - 2) = c && String.sub b (String.length b - 2) 2 = "by" then Disamb else Unknown
  | [[b;"ś"];[c;"by";"ś"]] -> if String.sub b 0 (String.length b - 2) = c && String.sub b (String.length b - 2) 2 = "by" then Disamb else Unknown
  | [[b;"śmy"];[c;"by";"śmy"]] -> if String.sub b 0 (String.length b - 2) = c && String.sub b (String.length b - 2) 2 = "by" then Disamb else Unknown
  | [[b;"ście"];[c;"by";"ście"]] -> if String.sub b 0 (String.length b - 2) = c && String.sub b (String.length b - 2) 2 = "by" then Disamb else Unknown
  | [[a];[b;"m"];[c;"em"]] ->
...
let count_variants stats corpus =
  Xlist.fold corpus stats (fun stats (conll_sentence, text, orig, tokens) ->
    let eniam_tokens = ENIAMtokenizer.parse text in
    let eniam_tokens = convert_eniam_tokens [] eniam_tokens in
    count_variants_par conll_sentence.ENIAMsubsyntaxTypes.id text stats eniam_tokens)
...
let string_of_nkjp_token_orths2 l =
  String.concat "\";\"" (Xlist.map l (fun n -> if n.north = "" then "*" ^ n.nlemma ^ "*" else n.north))

let string_of_nkjp_token_lemma l =
  String.concat "; " (Xlist.map l (fun n -> n.nlemma ^ "," ^ n.ncat ^ "," ^ String.concat ":" n.ninterp))
...
let string_of_atoken2 = function
    AT(t,l) -> Printf.sprintf "AT(%s,[%s])" (ENIAMtokens.string_of_token t.token) (string_of_nkjp_token_lemma l)
  | AV(tl,l) ->
      Printf.sprintf "AV([ [%s] ],[%s])"
        (String.concat "] | [" (Xlist.map tl (fun l -> String.concat " " (Xlist.map l (fun t -> ENIAMtokens.string_of_token t.token)))))
        (string_of_nkjp_token_lemma l)
  | AR(stat,tl,l) ->
      Printf.sprintf "AR(%s,[ %s ],[%s])" stat
        (String.concat " | " (Xlist.map tl (fun l -> String.concat " " (Xlist.map l (fun t -> ENIAMtokens.string_of_token t.token)))))
        (string_of_nkjp_token_lemma l)
...
let prepare_e_pat s =
  {nid=(-1);(*nsent=Space;*)north=s;nlemma="";ncat="";ninterp=[]}
...
let load_rules filename =
  ...
  | l -> failwith ("load_rules " ^ filename ^ ": " ^ String.concat "\t" l)))
...
let space_rules = Xlist.map [
  ...
  "nkjp-correct",["pomaga";"my"],["pomaga my"];
  "nkjp-correct",["dla";"tego"],["dla tego"];
  "both-correct",["19 05 01"],["19";"05";"01"];
  "both-correct",["270-300 mln"],["270";"-";"300";"mln"];
  "both-correct",["2,5 tys."],["2";",";"5";"tys";"."];
  "both-correct",["300-350 tys."],["300";"-";"350";"tys";"."];
  "nkjp-correct",["a";"j";"e";"n";"t";"a"],["a j e n t a"];
  "nkjp-correct",["bar";"dziej"],["bar dziej"];
...
let disambiguate_letni = function
    AR("letni",variants,l) ->
      ...
      let variants = Xlist.fold variants [] (fun variants seq ->
        ...
        | _ :: _ :: {token=Compound("day-month",_)} :: _ -> variants
        ...
        | _ :: _ :: {token=Lemma(_,"obj-id",_)} :: _ -> variants
        | _ :: _ :: {token=Lemma(_,"building-number",_)} :: _ -> variants
        | _ :: _ :: {token=RomanDig(_,"month")} :: _ -> variants
        | _ :: _ :: {token=Lemma(_,"interj",_)} :: _ -> variants
        ...
        | _ :: _ :: t :: _ ->
            print_endline ("disambiguate_letni: " ^ ENIAMtokens.string_of_token t.token);
            seq :: variants
...
let rec annotate_paragraph name paragraph l = function
    et :: ets ->
      ...
      with Not_found -> (try
        (* print_endline ("annotate_paragraph 3: "); *)
        let m,ets,l = match_tys (et :: ets) l in
        (* print_endline ("annotate_paragraph 4: "); *)
        m :: annotate_paragraph name paragraph l ets
      with Not_found -> (try
        (* print_endline ("annotate_paragraph 5: "); *)
        let m,ets,l = annotate_apply_rules (et :: ets) l rules in
        (* print_endline ("annotate_paragraph 6: "); *)
        m :: annotate_paragraph name paragraph l ets
...
let validate_segmentation stats corpus =
  Xlist.fold corpus stats (fun stats (conll_sentence, text, orig, tokens) ->
    let tokens = simple_allign_conll tokens in
    let eniam_tokens = ENIAMtokenizer.parse text in
    let eniam_tokens = convert_eniam_tokens [] eniam_tokens in
    match_and_combine conll_sentence.ENIAMsubsyntaxTypes.id text stats tokens eniam_tokens)
...
let set_sent sent t = Token t(* FIXME: todo *)
(*  match sent with
...
let rec match_interp = function
    "congr" :: li, "congr.rec" :: ln -> "congr" :: (match_interp (li,ln))
  | "rec" :: li, "congr.rec" :: ln -> "congr" :: (match_interp (li,ln))
  | "col" :: li, "col.ncol" :: ln -> "col" :: (match_interp (li,ln))
  | "ncol" :: li, "col.ncol" :: ln -> "ncol" :: (match_interp (li,ln))
  | i :: li, n :: ln ->
      if i = n then i :: (match_interp (li,ln)) else raise Not_found
  | [],[] -> []
  | _ -> failwith "match_interp"

let correct_num = function
    [n;c;"m1";a;_] -> [n;c;"m1";a]
  | [n;c;"m2";a;_] -> [n;c;"m2";a]
  | [n;c;"m3";a;_] -> [n;c;"m3";a]
  | [n;c;"f";a;_] -> [n;c;"f";a]
...
let match_interps cat interps ninterp =
  Xlist.fold interps [] (fun interps interp ->
    Xlist.fold (Xlist.multiply_list interp) interps (fun interps interp ->
      let interp = if cat = "num" then correct_num interp else interp in
      if Xlist.size interp <> Xlist.size ninterp then interps else
      try match_interp (interp,ninterp) :: interps with Not_found -> interps))

type matching =
    Matched of token_env * int
  | Accepted of token_env * int
  | NotMatched

let merge_token text = function
  | {token=Other s} as t,[{ncat="interp"} as n] -> Accepted({t with token=Interp s},n.nid)
  | {token=Interp _} as t,[] -> Accepted(t,-1)
  | {token=Symbol _} as t,[] -> Accepted(t,-1)
  | {token=Interp _} as t,[n] -> if n.ncat="interp" then Matched(t,n.nid) else NotMatched
  | {token=Symbol _} as t,[n] -> if n.ncat="interp" then Matched(t,n.nid) else NotMatched
  | {token=AllSmall _} as t,[n] -> Accepted(t,n.nid)
  | {token=SmallLetter _} as t,[n] -> Accepted(t,n.nid)
  | {token=FirstCap _} as t,[n] -> Accepted(t,n.nid)
  | {token=AllCap _} as t,[n] -> Accepted(t,n.nid)
  | {token=CapLetter _} as t,[n] -> Accepted(t,n.nid)
  | {token=SomeCap _} as t,[n] -> Accepted(t,n.nid)
  | {token=Dig(_,"email")} as t,[{ncat="subst"} as n] -> Accepted(t,n.nid)
  | {token=Dig(_,"url")} as t,[{ncat="subst"} as n] -> Accepted(t,n.nid)
  | {token=Dig(_,"4dig")},[_] -> NotMatched
  | {token=Dig(_,"pref3dig")},[_] -> NotMatched
  | {token=Dig(_,"dig")},[_] -> NotMatched
  | {token=Dig(_,"2dig")},[_] -> NotMatched
  | {token=Dig(_,"3dig")},[_] -> NotMatched
  | {token=Dig(_,"minute")},[_] -> NotMatched
  | {token=Dig(_,"month")},[_] -> NotMatched
  | {token=Dig(_,"hour")} as t,[{ncat="adj";ninterp=[_;_;"f";_]} as n] -> Accepted(t,n.nid)
  | {token=Dig(_,"hour")} as t,[{ncat="subst"} as n] -> Accepted(t,n.nid)
  | {token=Dig(_,"day")} as t,[{ncat="adj";ninterp=[_;_;"m3";_]} as n] -> Accepted(t,n.nid)
  | {token=Dig(_,"year")} as t,[{ncat="adj";ninterp=[_;_;"m3";_]} as n] -> Accepted(t,n.nid)
  | {token=Dig(_,"hour")},[{ncat="num"}] -> NotMatched
  | {token=Dig(_,"day")},[{ncat="num"}] -> NotMatched
  | {token=Dig(_,"year")},[{ncat="num"}] -> NotMatched
  | {token=Dig(_,"hour")},[{ncat="dig"}] -> NotMatched
  | {token=Dig(_,"day")},[{ncat="dig"}] -> NotMatched
  | {token=Dig(_,"year")},[{ncat="dig"}] -> NotMatched
  | {token=Dig(_,"hour")},[{ncat="adj"}] -> NotMatched
  | {token=Dig(_,"day")},[{ncat="adj"}] -> NotMatched
  | {token=Dig(_,"year")},[{ncat="adj"}] -> NotMatched
  | {token=Dig(_,"intnum")} as t,[{ncat="num"} as n] -> Accepted(t,n.nid)
  | {token=Dig(_,"intnum")} as t,[{ncat="adj"} as n] -> Accepted(t,n.nid) (* oznacza ordnum *)
  | {token=Dig(_,"intnum")} as t,[{ncat="subst"} as n] -> Accepted(t,n.nid)
  | {token=Dig(_,"intnum")},[{ncat="dig"}] -> NotMatched
  | {token=Dig(_,"realnum")} as t,[{ncat="num"} as n] -> Accepted(t,n.nid)
  | {token=Dig(_,"realnum")} as t,[{ncat="adj"} as n] -> Accepted(t,n.nid) (* błąd w korpusie *)
  | {token=Dig(_,"realnum")},[{ncat="dig"}] -> NotMatched
  | {token=RomanDig _},[{ncat="conj"}] -> NotMatched
  | {token=RomanDig _},[{ncat="qub"}] -> NotMatched
  | {token=RomanDig _},[{ncat="brev"}] -> NotMatched
  | {token=RomanDig _},[{ncat="interj"}] -> NotMatched
  | {token=RomanDig _},[{ncat="ppron12"}] -> NotMatched
  | {token=RomanDig _},[{ncat="subst"}] -> NotMatched
  | {token=RomanDig _} as t,[{ncat="adj"} as n] -> Accepted(t,n.nid)
  | {token=RomanDig _} as t,[{ncat="romandig"} as n] -> Accepted(t,n.nid)
  | {token=Compound("hour-minute",_)} as t,[{ncat="subst"} as n] -> Accepted(t,n.nid)
  | {token=Compound("hour-minute",_)} as t,[{ncat="adj";ninterp=[_;_;"f";_]} as n] -> Accepted(t,n.nid)
  | {token=Compound("match-result",_)},[_] -> NotMatched
  | {token=Compound("hour-minute",_)},[{ncat="dig"}] -> NotMatched
  | {token=Compound("day-month",_)},[{ncat="dig"}] -> NotMatched
  | {token=Compound("year-interval",_)},[{ncat="dig"}] -> NotMatched
  | {token=Compound("minute-interval",_)},[{ncat="dig"}] -> NotMatched
  | {token=Compound("intnum-interval",_)} as t,[{ncat="dig"} as n] -> Accepted(t,n.nid)
  | {token=Lemma(_,"obj-id",_)} as t,[{ncat="dig"} as n] -> Accepted(t,n.nid)
  | {token=Lemma(lemma,cat,interp)} as t,[n] ->
      let interps = match_interps cat interp n.ninterp in
      if lemma=n.nlemma && cat=n.ncat && Xlist.size interps > 0 then
        Matched({t with token=Lemma(lemma,cat,interp)},n.nid)
      else NotMatched
    (* t,[] -> Token t
  | t,[{ncat="brev"} as n] -> set_sent () {t with attrs=BrevLemma n.nlemma :: t.attrs}
  | t,[n] ->
      if n.nlemma = "+/-" then set_sent () t else
      if is_lemmatizable t.token then set_sent () {t with attrs=Disamb(n.nlemma,n.ncat,transform_nkjp_interp_simple n.ncat n.ninterp) :: t.attrs}
      else set_sent () t *)
  | t,l -> print_endline ("merge_token: " ^ (ENIAMtokens.string_of_token t.token) ^
      " [" ^ String.concat " | " (Xlist.map l (fun t -> t.nlemma ^ ":" ^ t.ncat ^ ":" ^ String.concat ":" t.ninterp)) ^ "] " ^ text); NotMatched
...
let rec merge_paragraph name text = function
    AT(t,l) ->
      (match merge_token text (t,l) with
              Matched(t,id) -> Token empty_token_env
            | Accepted(t,id) -> Token empty_token_env
            | NotMatched -> print_endline ("merge_paragraph 1: " ^ string_of_atoken2 (AT(t,l))); Token empty_token_env)
  | AV([[x;{orth="."} as y];[_]],[a;{north="."} as b]) -> merge_paragraph name text (AV([[x;y]],[a;b]))
  | AV([[_];[x;{orth="."} as y]],[a;{north="."} as b]) -> merge_paragraph name text (AV([[x;y]],[a;b]))
  | AV(variants,l) ->
      let variants2 = Xlist.fold variants [] (fun variants ets ->
        try
          let seq = List.rev (Xlist.fold (allign [] (ets,l)) [] (fun seq (t,l) ->
            match merge_token text (t,l) with
              Matched(t,id) -> (t,id) :: seq
            | Accepted(t,id) -> (t,id) :: seq
            | NotMatched -> raise Not_found)) in
          seq
        with Not_found -> variants) in
      if variants2 = [] then print_endline ("merge_paragraph 2: " ^ string_of_atoken2 (AV(variants,l)));
      ...
      (* Variant(Xlist.rev_map variants (fun ets -> Seq(Xlist.map (allign [] (ets,l)) (fun x -> let _ = merge_token text x in Token empty_token_env)))) *)
(*  | AR("tys",variants,l) -> Variant(Xlist.rev_map variants (fun ets -> Seq(Xlist.map (set_sent_list ets l) (fun t -> Token t))))
  ...
  | AR("nkjp-correct",variants,l) -> Seq(Xlist.map l (fun n -> set_sent () {empty_token_env with orth=n.north; token=Lemma(n.nlemma,n.ncat,[transform_nkjp_interp_simple n.ncat n.ninterp])})) (* FIXME: ustalenie beg len next *) *)
  | t -> Token empty_token_env(*failwith ("merge_paragraph: " ^ string_of_atoken t)*)

let test_annotate corpus =
  Xlist.iter corpus (fun (conll_sentence, text, orig, tokens) ->
    (* print_endline conll_sentence.ENIAMsubsyntaxTypes.id; *)
    let tokens = simple_allign_conll tokens in
    (* print_endline "test_annotate 0"; *)
    let eniam_tokens = ENIAMtokenizer.parse text in
    (* print_endline "test_annotate 01"; *)
    let eniam_tokens = convert_eniam_tokens [] eniam_tokens in
    (* print_endline "test_annotate 1"; *)
    let eniam_tokens = annotate_variants_par eniam_tokens in
    (* print_endline "test_annotate 2"; *)
    let m = annotate_paragraph conll_sentence.ENIAMsubsyntaxTypes.id text tokens eniam_tokens in
    (* print_endline "test_annotate 3"; *)
    check_annotation text m;
    (* print_annotation text m; *)
    let _ = List.rev (Xlist.rev_map m (merge_paragraph conll_sentence.ENIAMsubsyntaxTypes.id text)) in
    ())
