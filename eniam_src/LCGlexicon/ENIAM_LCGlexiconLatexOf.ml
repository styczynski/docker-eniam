...
let rec add_quantifiers_simple t = function
    [] -> t
  | (cat,s) :: l ->
    if ENIAM_LCGrenderer.count_avar (string_of_selector cat) t = 0 then add_quantifiers_simple t l
...
let _ =
  if Array.length Sys.argv < 2 then
    ...
    let lexicon = ENIAM_LCGlexiconParser.load_lexicon Sys.argv.(1) in
    let lexicon = List.rev (Xlist.rev_map lexicon ENIAM_LCGlexicon.assign_quantifiers) in
