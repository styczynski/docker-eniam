...
(*let _ =
  ...
  Xlist.iter test_strings (fun s ->
    print_endline ("\nTEST: " ^ s);
    let tokens = ENIAMinflexion.get_interpretations s in
...
let _ =
  ...
  Xlist.iter test_strings2 (fun (lemma,interp,feat,excl) ->
    print_endline ("\nTEST: " ^ lemma ^ " " ^ interp ^ 
      " [" ^ String.concat ";" (Xlist.map feat ENIAMmorphologyRules.string_of_star) ^ "] [" ^ 
      String.concat ";" (Xlist.map excl ENIAMmorphologyRules.string_of_star) ^ "]");
    let result = ENIAMinflexion.disambiguate feat excl (ENIAMinflexion.synthetize lemma interp) in
    (* print_endline (ENIAMtokenizer.xml_of tokens); *)
    print_endline (ENIAMinflexion.string_of_interpretations result ^ "\n\n"));
(*  print_endline "Testy użytkownika.";
  print_endline "Wpisz tekst i naciśnij ENTER, pusty tekst kończy.";
  let s = ref (read_line ()) in
  while !s <> "" do
