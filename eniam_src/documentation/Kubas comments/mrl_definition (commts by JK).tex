...
\todo[inline]{Czy nie powinno być $v(r)(v(x).time,v(y))$ zamiast $r(v(x).time,v(y))$? Albo $\langle v(x).time,v(y)\rangle \in v(r)?$}
...
\[\text{\bf ojciec}\equiv\lambda x\; x.\underline{\textcolor{red}{ptrc}}.ojciec=x \wedge \text{\bf mężczyzna}(x)\]

\todo[inline]{Czy odnośną cechą $y$-a jest posiadanie ojca? Trudno się tego domyślić na podstawie tego egzemplifikującego algorytmu.}
\todo[inline]{Koniunkt \textbf{mężczyzna}(x) jest zupełnie zbędny, każdy ojciec jest mężczyzną, równie dobrze można dodać \textbf{istota\_żywa}(x)}
Wtedy 
\[({\cal M},v\vDash \type{x}{ojciec}\wedge\hasName{y}{Józef}\wedge\prtc{x}{y}) \Longleftrightarrow \]
\[ v(x).\underline{\textcolor{red}{ptrc}}.ojciec=v(x) \wedge \text{\bf mężczyzna}(v(x))\; \wedge \]\[\wedge \;v(x).prtc=v(y) \wedge v(y).name=\text{``Józef''}\]
\[ \Longrightarrow v(y).ojciec=v(x) \wedge \text{\bf mężczyzna}(v(x)) \wedge v(y).name=\text{``Józef''}\]

\todo[inline]{\textbf{Brakująca relatywizacja}\\
$\prtc{x}{y}$ musi być relatywizowane do zadanego algorytmu znaczeniowego przysługującego $x$-owi. Dlaczego: Weźmy zdanie ,,Jan III to syn Jana II, wnuk Jana I''. Zgodnie z przedostatnią linią powyższej równoważności (koniunkt pierwszy) reprezentacja tego zdania zawiera jako podformułę $v(j3).prtc=v(j2) \wedge v(j3).prtc=v(j1)$, a zatem Jan II i Jan I musieliby być samą osobą.\\
}
\todo[inline]{\textbf{Funkcja one-to-many / pole $.ojciec$}\\
Traktowanie tak jak funkcji relacji bycia ojcem  $v(y).ojciec = x$ działa przypadkiem dla $ojciec$, bo każdy ma jednego biologicznego ojca, ale już dziadków ma się dwóch, więc dla $dziadek$ (i większości innych) jest już potrzebny porządny zapis relacyjny, np.\\$v(y).dziadek \ni v(x)$\\
$v(y).dziadek = \{v(x)\}$\\
lub normalnie
$dziadek(v(y), v(x))$\\
}
\todo[inline]{\textbf{Funkcja one-to-many / pole $.prtc$}\\
Traktowanie jak funkcji relacji uczestnictwa $.prtc$ działa przypadkiem w jedno-jednoznacznym uwikłaniu $v(x).prtc.ojciec = v(x)$, bo każdy ojciec jest jedynym ojcem swoich dzieci, ale tak naprawdę sam term $v(x).prtc$ jest niezdefiniowany, ponieważ nie zawsze da się wyznaczyć jedyną osobę, dla której $x$ jest ojcem. Dlatego zamiast $v(y).prtc = v(x)$ potrzebny jest porządny zapis relacyjny np.\\$v(y).prtc \ni v(x)$\\
$v(y).prtc = \{v(x)\}$\\
lub normalnie
$prtc(v(y), v(x))$}
...
\todo[inline]{Co z
\[\text{\bf dziadek}\equiv\lambda x\; x.prtc.\underline{matka}.ojciec=x \wedge \text{\bf mężczyzna}(x)\]
}
