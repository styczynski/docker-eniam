...
{\noindent\fboxsep5mm\framebox{\begin{minipage}[s][.9\textheight]{.94\textwidth}\vspace*{15mm}
\centering\parskip\baselineskip
%%%% Nazwa zadania:
\textbf{Z5} Efektywny parser składniowo-semantyczny

%%%% Nazwa podzadania
\textbf{Z5.2} Rozbudowa kategorialnego parsera składniowo-semantycznego

%%%% Miesiąc i nazwa kamienia milowego:
\textbf{M6} Kwerenda biblioteczna, utworzenie bazy jednostek leksykalnych i wstępnej listy zjawisk semantycznych, wstępna wersja koncepcji metaopisu semantycznego

%%%% okres:
Okres sprawozdawczy: październik – grudzień 2016

Partner: Instytut Podstaw Informatyki PAN, Warszawa

%%%% raport/zasób językowy/program (narzędzie językowe, usługa sieciowa lub system)
Typ: raport + zasób

\vfil
%%%% Data oddania kamienia:
31.12.2016, Warszawa

%%%% autor/osoba odpowiedzialna:
Autor: Wojciech Jaworski, Jakub Kozakoszczak

\vspace*{15mm}\end{minipage}}}
\thispagestyle{empty}
\newpage

%%%% Dalszy ciąg zależny od typu kamienia. W~wypadku raportu
%%%% dalej idzie raport.  W~wypadku zasobów językowych i~programów
%%%% dalej idzie opis według schematu:
...
%%%% Wyjaśnić w jaki sposób oddany przedmiot realizuje dany kamień milowy.
...
% W przypadku zasobu dodać krótkie informacje o:
% - formacie zasobu,
% - zawartości wraz z podstawową informacją statystyczną,
% - procedurze zapewnienia jakości i zmierzonej jakości (jeżeli została zmierzona),
% - ewentualnych publikacjach opisujących zasób lub dokumentacji.


% W przypadku programu:
% - przeznaczeniu programu, 
% - metodzie na jakiej jest oparty lub algorytmach (najważniejszych),
% - parametrach działania: jakościowych i statystycznych (np. szybkość przetwarzania),
% - modułach, od których program zależy,
...
\paragraph{Opis zjawiska} % (fold)
\label{sub:definicja_robocza1}


% subsubsection definicja_robocza (end)
...
\subsubsection{Niejawne argumenty semantyczne} % (fold)
\paragraph{Opis zjawiska} % (fold)
...
% subsubsection definicja_robocza (end)
\paragraph{Lista niejawnych argumentów semantycznych} % (fold)
\label{niejawne}
...
\subsubsection{Znaczenie relacji składniowych} % (fold)

\paragraph{Uogólniona intersektywność} % (fold)
...
%TODO \subsubsection{Nieintersektywność II typu}
...
% subsubsection faktywność (end)

\subsubsection{Cechy składniowo-semantyczne jednostek nieopisanych w zasobach} % (fold)
{
\paragraph{Opis problemu} % (fold)
\label{sub:opis_problemu}
}
% subsubsection opis_problemu (end)
...
\paragraph{Lista cech składniowo-semantycznych} % (fold)
\label{sub:cechy_ala_Walenty}

% subsubsection cechy_ala_Walenty (end)
...
\subsubsection{Oznaczanie miary i liczebności referenta} % (fold)
\paragraph{Opis zjawiska} % (fold)
\label{sub:opis_zjawiska4}
% subsubsection opis_zjawiska (end)
...
\paragraph{Użycie pojemnikowe} % (fold)
\label{pojemniki}
...
\paragraph{Podział liczebników ze względu na dokładność} % (fold)
...
\paragraph{Miara referentów niepoliczalnych} % (fold)
...
\subsubsection{Niedenotatywne funkcje języka} % (fold)
\paragraph{Opis zjawiska} % (fold)
\label{sub:opis_zjawiska2}
% subsubsection opis_zjawiska (end)
...
\subsubsection{Niejednoznaczność}
%TODO OPISAĆ
\begin{center}
...
\subsubsection{Zasoby istniejące i wymagające wytworzenia}
%TODO OPISAĆ
...
%%%% Zdać sprawę z~przeprowadzonej ewaluacji, jeśli była.  Podać
%%%% jakość zasobu lub wyników programu.



\section{Publikacje}

%%%% Wymienić publikacje opisujące zasób lub program, także dostępną dokumentację.
